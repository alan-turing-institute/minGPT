...
This is a fork of Andrej Karpathy's [minGPT](https://github.com/karpathy/minGPT) repository and the original README is below.
...
```bash
git clone git@github.com:alan-turing-institute/minGPT.git
cd minGPT
pip install -e .
```

We also use Poetry for managing dependencies, so you could install with Poetry too:
```
poetry install --all-extras
```

For development, please use the pre-commit hooks:
```
pre-commit install
```
